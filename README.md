...
2. Для каждого документа сначала попытайся получить карточку на pravo.by через mcp__crawl4ai__crawl. Для установления URL допускаются не более двух точных поисковых запросов. Поисковую выдачу используй только для поиска URL или регистрационного номера.
