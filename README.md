...
Chrome при этом визуально не открывается — всё работает headless.
...
с параметром referer равным корневому домену сайта. Если на pravo.by не удаётся
получить полный консолидированный текст документа (PDF недоступен, текст обрезан
или отсутствует актуальная редакция) — найди тот же документ на ilex.by и прочитай
его через инструмент mcp__crawl4ai__crawl_authenticated.
```
